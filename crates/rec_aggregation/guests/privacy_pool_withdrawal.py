...
# The claim-list digest uses its own domain tag, separate from the withdrawal
# statement digest and the individual claim identifier.
CLAIM_LIST_TAG_0 = CLAIM_LIST_TAG_0_PLACEHOLDER
CLAIM_LIST_TAG_1 = CLAIM_LIST_TAG_1_PLACEHOLDER
# The claim identifier follows the canonical withdrawal-claim preimage.
CLAIM_TAG_0 = CLAIM_TAG_0_PLACEHOLDER
CLAIM_TAG_1 = CLAIM_TAG_1_PLACEHOLDER
STMT_HEADER = 5
...
def withdrawal_claim_id(zero_0, zero_1, withdrawn_0, withdrawn_1, state_0, state_1, asp_0, asp_1, context_0, context_1, commitment_0, commitment_1, nullifier_0, nullifier_1):
    zero = [zero_0, zero_1]
    tag = [CLAIM_TAG_0, CLAIM_TAG_1]
    depth = [32, 0]
    withdrawn = [withdrawn_0, withdrawn_1]
    state_root = [state_0, state_1]
    asp_root = [asp_0, asp_1]
    context = [context_0, context_1]
    commitment = [commitment_0, commitment_1]
    existing_nullifier_hash = [nullifier_0, nullifier_1]
    claim_state = StackBuf(2)
    blake2s(tag, withdrawn, claim_state, counter=64, final=0)
    next_state = StackBuf(2)
    blake2s(state_root, depth, next_state, cv=claim_state, counter=128, final=0)
    claim_state = next_state
    next_state = StackBuf(2)
    blake2s(asp_root, depth, next_state, cv=claim_state, counter=192, final=0)
    claim_state = next_state
    next_state = StackBuf(2)
    blake2s(context, commitment, next_state, cv=claim_state, counter=256, final=0)
    claim_state = next_state
    claim_id = StackBuf(2)
    blake2s(existing_nullifier_hash, zero, claim_id, cv=claim_state, counter=288, final=1)
    return claim_id[0], claim_id[1]


def verify_withdrawal():
    zero = [0, 0]
    withdrawn = StackBuf(2)
    label = StackBuf(2)
    context = StackBuf(2)
    state_root = StackBuf(2)
    asp_root = StackBuf(2)
    new_nullifier_hash = StackBuf(2)
    existing_nullifier_hash = StackBuf(2)
    commitment = StackBuf(2)
    old_nullifier = StackBuf(2)
    new_nullifier = StackBuf(2)
    existing_secret = StackBuf(2)
    new_secret = StackBuf(2)
    nullifier_z = StackBuf(2)
    nullifier_inv = StackBuf(2)
    existing_low = StackBuf(2)
    existing_high = StackBuf(1)
    remaining = StackBuf(2)
    hint_witness(withdrawn, "withdrawn")
    hint_witness(label, "label")
    hint_witness(context, "context")
    hint_witness(state_root, "state_root")
    hint_witness(asp_root, "asp_root")
    hint_witness(new_nullifier_hash, "new_nullifier_hash")
    hint_witness(existing_nullifier_hash, "existing_nullifier_hash")
    hint_witness(commitment, "commitment")
    hint_witness(old_nullifier, "old_nullifier")
    hint_witness(new_nullifier, "new_nullifier")
    hint_witness(existing_secret, "existing_secret")
    hint_witness(new_secret, "new_secret")
    hint_witness(nullifier_z, "nullifier_z")
    hint_witness(nullifier_inv, "nullifier_inv")
    hint_witness(existing_low, "existing_low")
    hint_witness(existing_high, "existing_high")
    hint_witness(remaining, "remaining")
    assert withdrawn[1] == 0
    assert existing_low[1] == 0
    assert remaining[1] == 0
    assert existing_high[0] * existing_high[0] == existing_high[0]
    withdrawn_bits = HeapBuf(GEN ** 128)
    remaining_bits = HeapBuf(GEN ** 128)
    existing_bits = HeapBuf(GEN ** 128)
    hint_decompose_bits(withdrawn_bits, withdrawn[0], 128)
    hint_decompose_bits(remaining_bits, remaining[0], 128)
    hint_decompose_bits(existing_bits, existing_low[0], 128)
    withdrawn_rebuilt = 0
    remaining_rebuilt = 0
    existing_rebuilt = 0
    for i in unroll(0, 128):
        wb = withdrawn_bits[GEN ** i]
        rb = remaining_bits[GEN ** i]
        eb = existing_bits[GEN ** i]
        assert wb * wb == wb
        assert rb * rb == rb
        assert eb * eb == eb
        withdrawn_rebuilt += wb * COORD_BASIS[i]
        remaining_rebuilt += rb * COORD_BASIS[i]
        existing_rebuilt += eb * COORD_BASIS[i]
    assert withdrawn_rebuilt == withdrawn[0]
    assert remaining_rebuilt == remaining[0]
    assert existing_rebuilt == existing_low[0]
    carry = 0
    for i in unroll(0, 128):
        wb = withdrawn_bits[GEN ** i]
        rb = remaining_bits[GEN ** i]
        eb = existing_bits[GEN ** i]
        assert eb == wb + rb + carry
        carry = wb * rb + carry * (wb + rb)
    assert carry == existing_high[0]
    nullifier_d0 = old_nullifier[0] + new_nullifier[0]
    nullifier_d1 = old_nullifier[1] + new_nullifier[1]
    assert nullifier_z[0] * nullifier_z[0] == nullifier_z[0]
    assert nullifier_z[1] * nullifier_z[1] == nullifier_z[1]
    assert nullifier_d0 * nullifier_z[0] == 0
    assert nullifier_d1 * nullifier_z[1] == 0
    assert nullifier_d0 * nullifier_inv[0] == 1 + nullifier_z[0]
    assert nullifier_d1 * nullifier_inv[1] == 1 + nullifier_z[1]
    assert nullifier_z[0] * nullifier_z[1] == 0
    existing_precommitment = StackBuf(2)
    computed_existing_hash = StackBuf(2)
    blake2s(old_nullifier, zero, computed_existing_hash, counter=32, final=1)
    assert computed_existing_hash[0] == existing_nullifier_hash[0]
    assert computed_existing_hash[1] == existing_nullifier_hash[1]
    blake2s(old_nullifier, existing_secret, existing_precommitment)
    state_leaf = StackBuf(2)
    existing_value = [existing_low[0], existing_high[0]]
    existing_commitment_mid = StackBuf(2)
    blake2s(existing_value, label, existing_commitment_mid, counter=64, final=0)
    blake2s(existing_precommitment, zero, state_leaf, cv=existing_commitment_mid, counter=96, final=1)
    asp_leaf = label
    state_siblings = StackBuf(64)
    asp_siblings = StackBuf(64)
    state_bits = StackBuf(32)
    asp_bits = StackBuf(32)
    state_zero_0 = StackBuf(32)
    state_zero_1 = StackBuf(32)
    state_inv_0 = StackBuf(32)
    state_inv_1 = StackBuf(32)
    asp_zero_0 = StackBuf(32)
    asp_zero_1 = StackBuf(32)
    asp_inv_0 = StackBuf(32)
    asp_inv_1 = StackBuf(32)
    hint_witness(state_siblings, "state_siblings")
    hint_witness(asp_siblings, "asp_siblings")
    hint_witness(state_bits, "state_bits")
    hint_witness(asp_bits, "asp_bits")
    hint_witness(state_zero_0, "state_zero_0")
    hint_witness(state_zero_1, "state_zero_1")
    hint_witness(state_inv_0, "state_inv_0")
    hint_witness(state_inv_1, "state_inv_1")
    hint_witness(asp_zero_0, "asp_zero_0")
    hint_witness(asp_zero_1, "asp_zero_1")
    hint_witness(asp_inv_0, "asp_inv_0")
    hint_witness(asp_inv_1, "asp_inv_1")
    state_node = state_leaf
    asp_node = asp_leaf
    for i in unroll(0, 32):
        sbit = state_bits[i]
        abit = asp_bits[i]
        szero = state_zero_0[i] * state_zero_1[i]
        azero = asp_zero_0[i] * asp_zero_1[i]
        assert sbit * sbit == sbit
        assert abit * abit == abit
        assert state_zero_0[i] * state_zero_0[i] == state_zero_0[i]
        assert state_zero_1[i] * state_zero_1[i] == state_zero_1[i]
        assert asp_zero_0[i] * asp_zero_0[i] == asp_zero_0[i]
        assert asp_zero_1[i] * asp_zero_1[i] == asp_zero_1[i]
        assert state_siblings[2 * i] * state_zero_0[i] == 0
        assert state_siblings[2 * i + 1] * state_zero_1[i] == 0
        assert state_siblings[2 * i] * state_inv_0[i] == 1 + state_zero_0[i]
        assert state_siblings[2 * i + 1] * state_inv_1[i] == 1 + state_zero_1[i]
        assert asp_siblings[2 * i] * asp_zero_0[i] == 0
        assert asp_siblings[2 * i + 1] * asp_zero_1[i] == 0
        assert asp_siblings[2 * i] * asp_inv_0[i] == 1 + asp_zero_0[i]
        assert asp_siblings[2 * i + 1] * asp_inv_1[i] == 1 + asp_zero_1[i]
        sd0 = state_node[0] + state_siblings[2 * i]
        sd1 = state_node[1] + state_siblings[2 * i + 1]
        sm0 = sbit * sd0
        sm1 = sbit * sd1
        left = [state_node[0] + sm0, state_node[1] + sm1]
        right = [state_siblings[2 * i] + sm0, state_siblings[2 * i + 1] + sm1]
        snext = StackBuf(2)
        blake2s(left, right, snext)
        state_node = [snext[0] + szero * (snext[0] + state_node[0]), snext[1] + szero * (snext[1] + state_node[1])]
        ad0 = asp_node[0] + asp_siblings[2 * i]
        ad1 = asp_node[1] + asp_siblings[2 * i + 1]
        am0 = abit * ad0
        am1 = abit * ad1
        aleft = [asp_node[0] + am0, asp_node[1] + am1]
        aright = [asp_siblings[2 * i] + am0, asp_siblings[2 * i + 1] + am1]
        anext = StackBuf(2)
        blake2s(aleft, aright, anext)
        asp_node = [anext[0] + azero * (anext[0] + asp_node[0]), anext[1] + azero * (anext[1] + asp_node[1])]
    assert state_node[0] == state_root[0]
    assert state_node[1] == state_root[1]
    assert asp_node[0] == asp_root[0]
    assert asp_node[1] == asp_root[1]
    computed_output = StackBuf(2)
    blake2s(new_nullifier, zero, computed_output, counter=32, final=1)
    assert computed_output[0] == new_nullifier_hash[0]
    assert computed_output[1] == new_nullifier_hash[1]
    new_precommitment = StackBuf(2)
    blake2s(new_nullifier, new_secret, new_precommitment)
    computed_commitment_mid = StackBuf(2)
    blake2s(remaining, label, computed_commitment_mid, counter=64, final=0)
    computed_commitment = StackBuf(2)
    blake2s(new_precommitment, zero, computed_commitment, cv=computed_commitment_mid, counter=96, final=1)
    assert computed_commitment[0] == commitment[0]
    assert computed_commitment[1] == commitment[1]
    w0 = withdrawn[0]
    w1 = withdrawn[1]
    s0 = state_root[0]
    s1 = state_root[1]
    ...
    m0 = commitment[0]
    m1 = commitment[1]
    n0 = existing_nullifier_hash[0]
    n1 = existing_nullifier_hash[1]
    claim_0, claim_1 = withdrawal_claim_id(zero[0], zero[1], w0, w1, s0, s1, a0, a1, c0, c1, m0, m1, n0, n1)
    return claim_0, claim_1


def bind_count(count_g, count_word):
    bits = HeapBuf(GEN ** 16)
    hint_decompose_bits(bits, count_word, 16)
    rebuilt_word = 0
    rebuilt_g = 1
    for i in unroll(0, 16):
        bit = bits[GEN ** i]
        assert bit * bit == bit
        rebuilt_word += bit * COORD_BASIS[i]
        factor = GEN ** (2 ** i)
        rebuilt_g *= 1 + bit * (factor + 1)
    assert rebuilt_word == count_word
    assert rebuilt_g == count_g


def hash_claims(count_g, count_word, claim_table):
    state_table = HeapBuf(65536)
    base = GEN ** 2
    state_table[base] = CLAIM_LIST_TAG_0
    state_table[base * GEN] = CLAIM_LIST_TAG_1
    count_block = StackBuf(2)
    count_block[0] = count_word
    count_block[1] = 0
    blake2s(state_table[base:base + 2], count_block, state_table[base * GEN ** 2:base * GEN ** 2 + 2])
    for xi in mul_range(1, count_g):
        ident = StackBuf(2)
        ident[0] = claim_table[xi * xi]
        ident[1] = claim_table[xi * xi * GEN]
        blake2s(state_table[base * GEN ** 2 * (xi * xi):base * GEN ** 2 * (xi * xi) + 2], ident, state_table[base * GEN ** 2 * ((xi * GEN) * (xi * GEN)):base * GEN ** 2 * ((xi * GEN) * (xi * GEN)) + 2])
    final_offset = base * GEN ** 2 * (count_g * count_g)
    return state_table[final_offset], state_table[final_offset * GEN]


def statement_digest(seed_0, seed_1, count_g, claim_hash_0, claim_hash_1, defer):
    cells = StackBuf(4 * STMT_BLOCKS)
    cells[0] = STMT_TAG_0
    cells[1] = STMT_TAG_1
    header = [seed_0, seed_1, count_g, claim_hash_0, claim_hash_1]
    ...
    state = StackBuf(2)
    blake2s(cells[0:2], cells[2:4], state, counter=64, final=1 // STMT_BLOCKS)
    for b in unroll(1, STMT_BLOCKS):
        nxt = StackBuf(2)
        blake2s(cells[4 * b:4 * b + 2], cells[4 * b + 2:4 * b + 4], nxt, cv=state, counter=64 * (b + 1), final=(b + 1) // STMT_BLOCKS)
        state = nxt
...
def main():
    ...
    n_claims_g = meta[0]
    n_children_g = meta[1]
    n_claims_word = meta[2]
    bind_count(n_claims_g, n_claims_word)
    assert log(n_claims_g) < MAX_KEYS
    assert log(n_children_g) < MAX_CHILDREN + 1
    if n_children_g == 1:
        assert n_claims_g == GEN
        claim_0, claim_1 = verify_withdrawal()
        defer_stmt = HeapBuf(DEFER_STMT_CELLS)
        list_state = StackBuf(2)
        list_state[0] = CLAIM_LIST_TAG_0
        list_state[1] = CLAIM_LIST_TAG_1
        count_block = StackBuf(2)
        count_block[0] = n_claims_word
        count_block[1] = 0
        list_after_count = StackBuf(2)
        blake2s(list_state, count_block, list_after_count)
        claim_id = StackBuf(2)
        claim_id[0] = claim_0
        claim_id[1] = claim_1
        list_after_id = StackBuf(2)
        blake2s(list_after_count, claim_id, list_after_id)
        claim_hash_0 = list_after_id[0]
        claim_hash_1 = list_after_id[1]
        ...
        own_0, own_1 = statement_digest(seed_0, seed_1, n_claims_g, claim_hash_0, claim_hash_1, defer_stmt * GEN ** 0)
        p = 1
        p[1] = own_0
        p[GEN] = own_1
        return
    else:
        assert n_children_g != GEN
        assert n_claims_g != 1
        claim_table = HeapBuf(n_claims_g * n_claims_g)
        for xi in mul_range(1, n_claims_g):
            ident = StackBuf(2)
            hint_witness(ident, "claim_ids")
            claim_table[xi * xi] = ident[0]
            claim_table[xi * xi * GEN] = ident[1]
        claim_hash_0, claim_hash_1 = hash_claims(n_claims_g, n_claims_word, claim_table * GEN ** 0)
        ...
        written[GEN ** 0] = 1
        cover = HeapBuf(n_claims_g)
        for xc in mul_range(1, n_children_g):
            base = written[xc]
            child_count = StackBuf(2)
            hint_witness(child_count, "child_count")
            bind_count(child_count[0], child_count[1])
            assert log(child_count[0]) < MAX_KEYS
            assert child_count[0] != 1
            child_indices = HeapBuf(child_count[0])
            for xi in mul_range(1, child_count[0]):
                idx = StackBuf(1)
                hint_witness(idx, "child_index")
                assert log(idx[0]) < log(n_claims_g)
                if xi != 1:
                    assert log(child_indices[xi * INV_GEN]) < log(idx[0])
                child_indices[xi] = idx[0]
                cover[idx[0]] = base * xi
            written[xc * GEN] = base * child_count[0]
            child_hash_table = HeapBuf(child_count[0] * child_count[0] * GEN ** 8)
            child_hash_base = GEN ** 2
            child_hash_table[child_hash_base] = CLAIM_LIST_TAG_0
            child_hash_table[child_hash_base * GEN] = CLAIM_LIST_TAG_1
            child_count_block = StackBuf(2)
            child_count_block[0] = child_count[1]
            child_count_block[1] = 0
            blake2s(child_hash_table[child_hash_base:child_hash_base + 2], child_count_block, child_hash_table[child_hash_base * GEN ** 2:child_hash_base * GEN ** 2 + 2])
            for xi in mul_range(1, child_count[0]):
                ident = StackBuf(2)
                idx = child_indices[xi]
                ident[0] = claim_table[idx * idx]
                ident[1] = claim_table[idx * idx * GEN]
                child_hash_in = child_hash_base * GEN ** 2 * (xi * xi)
                child_hash_out = child_hash_base * GEN ** 2 * ((xi * GEN) * (xi * GEN))
                blake2s(child_hash_table[child_hash_in:child_hash_in + 2], ident, child_hash_table[child_hash_out:child_hash_out + 2])
            child_hash_final = child_hash_base * GEN ** 2 * (child_count[0] * child_count[0])
            xd = xc ** DEFER_STMT_CELLS
            hint_witness(child_carried[xd:xd + DEFER_STMT_CELLS], "child_defer")
            pi_0, pi_1 = statement_digest(seed_0, seed_1, child_count[0], child_hash_table[child_hash_final], child_hash_table[child_hash_final * GEN], child_carried * xd)
            x2 = xc * xc
            child_pi[x2] = pi_0
            child_pi[x2 * GEN] = pi_1
            verify_sub(pi_0, pi_1, seed_0, seed_1, g_logs_pow2, g_squares, child_fresh * xc ** DEFER_SIZE)
        assert written[n_children_g] == n_claims_g
        defer_stmt = HeapBuf(DEFER_STMT_CELLS)
