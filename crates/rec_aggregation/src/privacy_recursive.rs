//! Self-recursive Privacy Pool withdrawal aggregation.
//!
//! Every node uses `guests/privacy_pool_withdrawal.py`. A leaf proves the fixed
//! depth-32 withdrawal relation. A parent verifies two through sixteen proofs
//! of that same program and publishes the digest of the complete public
//! withdrawal list, ordered by existing nullifier hash.
//!
//! The recursion verifier carries the same three deferred polynomial claims as
//! the existing XMSS aggregation implementation. The root discharges those
//! claims during native verification.

use std::collections::BTreeMap;
use std::ops::Range;

use lean_compiler::{compile, parse_with_replacements};
use lean_vm::cpu::{Program, proof_shape, prove, verify};
...
const PRIVACY_CLAIM_LABEL: &[u8] = b"leanvm-b/pp-withdrawal-claim/v1";
const PRIVACY_CLAIM_LIST_LABEL: &[u8] = b"leanvm-b/pp-claim-list/v1";
const PRIVACY_STATEMENT_LABEL: &[u8] = b"leanvm-b/pp-withdraw-stmt/v1";
...
const XMSS_STATEMENT_HEADER: usize = 9;
const STATEMENT_HEADER: usize = 5;
...
fn statement_digest(
    ...
    let header: [F192; XMSS_STATEMENT_HEADER] = [
        seed[0],
        seed[1],
        count(n_keys),
        pubkeys_hash[0],
...
/// The guest's `COORD_KIND_*` code for a coordinate (`guests/privacy_pool_withdrawal.py`),
...
/// Config + hints for the recursion guest (`guests/privacy_pool_withdrawal.py`), built
...
fn placeholder_map(kbc: usize) -> BTreeMap<String, String> {
    ...
    let tag = label_tag(PRIVACY_STATEMENT_LABEL);
    ps("STMT_TAG_0", u(val16(&tag[..16])).to_string());
    ps("STMT_TAG_1", u(val16(&tag[16..])).to_string());
    let claim_tag = label_tag(PRIVACY_CLAIM_LIST_LABEL);
    ps("CLAIM_LIST_TAG_0", u(val16(&claim_tag[..16])).to_string());
    ps("CLAIM_LIST_TAG_1", u(val16(&claim_tag[16..])).to_string());
    let claim_id_tag = label_tag(PRIVACY_CLAIM_LABEL);
    ps("CLAIM_TAG_0", u(val16(&claim_id_tag[..16])).to_string());
    ps("CLAIM_TAG_1", u(val16(&claim_id_tag[16..])).to_string());
...
fn compile_guest(kbc: usize) -> Program {
    ...
    let guest = compile(
        &parse_with_replacements(include_str!("../guests/privacy_pool_withdrawal.py"), &replacements)
...
/// The withdrawal workload's one self-recursive guest. It is separate from
/// the XMSS module's implementation and owns its own bytecode fixed point.
pub fn privacy_guest() -> &'static Program {
    unified_guest()
}

pub fn privacy_program_identity() -> [u8; 32] {
    let mut bytes = Vec::with_capacity(48);
    for cell in lean_vm::cpu::fs_seed(unified_guest()) {
        bytes.extend_from_slice(&cell.c0.to_le_bytes());
        bytes.extend_from_slice(&cell.c1.to_le_bytes());
        bytes.extend_from_slice(&cell.c2.to_le_bytes());
    }
    primitives::blake2s::hash(&bytes)
}

use crate::privacy_pool::{PrivacyError, Withdrawal, WithdrawalPublic};

fn privacy_cells(value: &[u8; 32]) -> [F192; 2] {
    pack_hash_state(value)
}

fn privacy_bits(index: u32) -> Vec<F192> {
    (0..32)
        .map(|bit| F192::new(((index >> bit) & 1) as u64, 0, 0))
        .collect()
}

fn check_privacy_list(withdrawals: &[WithdrawalPublic]) -> Result<(), PrivacyError> {
    if withdrawals.iter().any(|withdrawal| {
        withdrawal.state_tree_depth as usize != crate::privacy_pool::TREE_DEPTH
            || withdrawal.asp_tree_depth as usize != crate::privacy_pool::TREE_DEPTH
    }) {
...
fn privacy_claim_ids(withdrawals: &[WithdrawalPublic]) -> Result<Vec<[u8; 32]>, PrivacyError> {
    check_privacy_list(withdrawals)?;
    Ok(withdrawals
        .iter()
        .map(crate::privacy_pool::withdrawal_claim_id)
        .collect())
}

fn privacy_list_digest(withdrawals: &[WithdrawalPublic]) -> Result<[u8; 32], PrivacyError> {
    let ids = privacy_claim_ids(withdrawals)?;
    let tag = label_tag(PRIVACY_CLAIM_LIST_LABEL);
    let mut state = compress2(privacy_cells(&tag), [F192::new(ids.len() as u64, 0, 0), F192::ZERO]);
    for id in &ids {
        state = compress2(state, privacy_cells(id));
    }
    ...
    Ok(bytes.try_into().expect("two F192 cells are 32 bytes"))
}

fn privacy_statement_digest(
    withdrawals: &[WithdrawalPublic],
    defer: &DeferredClaim,
) -> Result<[F192; 2], PrivacyError> {
    let list = privacy_list_digest(withdrawals)?;
    let seed = lean_vm::cpu::fs_seed(unified_guest());
    let header = [
        seed[0],
        seed[1],
        count(withdrawals.len()),
        privacy_cells(&list)[0],
        privacy_cells(&list)[1],
    ];
    let mut cells = defer.cells();
    if !cells.len().is_multiple_of(2) {
    ...
    Ok(tagged_hash(
        ...
        header
            .iter()
            .flat_map(|cell| [cell.c0, cell.c1])
            .chain(cells.iter().flat_map(|cell| [cell.c0, cell.c1, cell.c2])),
    ))
}

fn digest_bytes(cells: [F192; 2]) -> [u8; 32] {
    let mut bytes = [0u8; 32];
    bytes[..8].copy_from_slice(&cells[0].c0.to_le_bytes());
    bytes[8..16].copy_from_slice(&cells[0].c1.to_le_bytes());
    bytes[16..24].copy_from_slice(&cells[1].c0.to_le_bytes());
    bytes[24..].copy_from_slice(&cells[1].c1.to_le_bytes());
    bytes
}

fn digest_cells(bytes: [u8; 32]) -> [F192; 2] {
    privacy_cells(&bytes)
}

fn set_withdrawal_hints(program: &mut Program, withdrawal: &Withdrawal) {
    let cells = privacy_cells;
    program.set_witness("withdrawn", vec![withdrawal.withdrawn.cells().to_vec()]);
    program.set_witness("label", vec![cells(&withdrawal.label).to_vec()]);
    program.set_witness("context", vec![cells(&withdrawal.context).to_vec()]);
    program.set_witness("state_root", vec![cells(&withdrawal.state_root).to_vec()]);
    program.set_witness("asp_root", vec![cells(&withdrawal.asp_root).to_vec()]);
    program.set_witness(
        "new_nullifier_hash",
        vec![cells(&withdrawal.new_nullifier_hash).to_vec()],
    );
    program.set_witness(
        "existing_nullifier_hash",
        vec![cells(&crate::privacy_pool::h1(&withdrawal.old_nullifier)).to_vec()],
    );
    program.set_witness("commitment", vec![cells(&withdrawal.new_commitment_hash).to_vec()]);
    program.set_witness("old_nullifier", vec![cells(&withdrawal.old_nullifier).to_vec()]);
    program.set_witness("new_nullifier", vec![cells(&withdrawal.new_nullifier).to_vec()]);
    program.set_witness("existing_secret", vec![cells(&withdrawal.existing_secret).to_vec()]);
    program.set_witness("new_secret", vec![cells(&withdrawal.new_secret).to_vec()]);
    let old = cells(&withdrawal.old_nullifier);
    let new = cells(&withdrawal.new_nullifier);
    let differences = [old[0] + new[0], old[1] + new[1]];
    program.set_witness(
        "nullifier_z",
        vec![
            differences
                .iter()
                .map(|value| F192::new((*value == F192::ZERO) as u64, 0, 0))
                .collect(),
        ],
    );
    program.set_witness(
        "nullifier_inv",
        vec![differences.iter().map(|value| value.inv()).collect()],
    );
    program.set_witness(
        "existing_low",
        vec![
            crate::privacy_pool::U128(withdrawal.existing_value.low)
                .cells()
                .to_vec(),
        ],
    );
    program.set_witness(
        "existing_high",
        vec![vec![F192::new(withdrawal.existing_value.high as u64, 0, 0)]],
    );
    program.set_witness("remaining", vec![withdrawal.remaining.cells().to_vec()]);
    for (name, path) in [("state", &withdrawal.state_path), ("asp", &withdrawal.asp_path)] {
        program.set_witness(
            format!("{name}_siblings"),
            vec![path.siblings.iter().flat_map(cells).collect()],
        );
        program.set_witness(format!("{name}_bits"), vec![privacy_bits(path.index)]);
        for cell in 0..2 {
            let values = path.siblings.iter().map(cells).collect::<Vec<_>>();
            program.set_witness(
                format!("{name}_zero_{cell}"),
                vec![
                    values
                        .iter()
                        .map(|value| F192::new((value[cell] == F192::ZERO) as u64, 0, 0))
                        .collect(),
                ],
            );
            program.set_witness(
                format!("{name}_inv_{cell}"),
                vec![values.iter().map(|value| value[cell].inv()).collect()],
            );
        }
    }
}

fn prove_privacy_leaf_tampered(
    withdrawal: &Withdrawal,
    log_inv_rate: usize,
    tamper: impl FnOnce(&mut Program),
) -> Result<PrivacyAggregate, PrivacyError> {
    crate::privacy_pool::validate_withdrawal(withdrawal)
        .map_err(|error| PrivacyError::InvalidWithdrawal(error.to_string()))?;
    let public = withdrawal.public();
    let defer = DeferredClaim::leaf();
    let expected = privacy_statement_digest(&[public], &defer)?;
    let mut program = unified_guest().clone();
    program.set_witness("fs_seed", vec![lean_vm::cpu::fs_seed(unified_guest()).to_vec()]);
    program.set_witness("meta", vec![vec![count(1), count(0), F192::new(1, 0, 0)]]);
    set_withdrawal_hints(&mut program, withdrawal);
    program.set_witness(
        "leaf_defer",
        vec![vec![defer.bytecode_value, defer.matrix_a_value, defer.matrix_b_value]],
    );
    tamper(&mut program);
    program.min_log_committed = MU_MIN;
    let (proof, _) = prove(&program, expected, log_inv_rate);
    verify(unified_guest(), &expected, &proof).map_err(PrivacyError::Vm)?;
    Ok(PrivacyAggregate {
        withdrawals: vec![public],
        list_digest: privacy_list_digest(&[public])?,
        statement_digest: digest_bytes(expected),
        defer,
        proof,
    })
}

fn prove_privacy_leaf(withdrawal: &Withdrawal, log_inv_rate: usize) -> Result<PrivacyAggregate, PrivacyError> {
    prove_privacy_leaf_tampered(withdrawal, log_inv_rate, |_| {})
}

fn check_privacy_parent_arity(child_count: usize) -> Result<(), PrivacyError> {
    if (2..=16).contains(&child_count) {
        Ok(())
    } else {
        Err(PrivacyError::InvalidList("parent arity must be in 2..=16".into()))
    }
}

#[derive(Clone, Debug)]
pub struct PrivacyAggregate {
    pub withdrawals: Vec<WithdrawalPublic>,
...
impl PrivacyAggregate {
    pub fn prove_leaf(withdrawal: &Withdrawal, log_inv_rate: usize) -> Result<Self, PrivacyError> {
        prove_privacy_leaf(withdrawal, log_inv_rate)
    }

    pub fn aggregate(children: &[Self], log_inv_rate: usize) -> Result<Self, PrivacyError> {
        Self::aggregate_tampered(children, log_inv_rate, |_| {})
    }

    fn aggregate_tampered(
        children: &[Self],
        log_inv_rate: usize,
        tamper: impl FnOnce(&mut Hints),
    ) -> Result<Self, PrivacyError> {
        check_privacy_parent_arity(children.len())?;
        let mut withdrawals = children
            .iter()
            .flat_map(|child| child.withdrawals.iter().copied())
            .collect::<Vec<_>>();
        withdrawals.sort_by_key(|withdrawal| withdrawal.existing_nullifier_hash);
        check_privacy_list(&withdrawals)?;
        let ids = privacy_claim_ids(&withdrawals)?;
        let guest = unified_guest();
        let mut verified = Vec::with_capacity(children.len());
        for child in children {
            if privacy_list_digest(&child.withdrawals)? != child.list_digest {
                return Err(PrivacyError::InvalidProof(
                    "child withdrawal list digest mismatch".into(),
                ));
            }
            let pi = digest_cells(child.statement_digest);
            let child_expected = privacy_statement_digest(&child.withdrawals, &child.defer)?;
            if child_expected != pi {
                return Err(PrivacyError::InvalidProof("child statement digest mismatch".into()));
            }
            let (summary, ops) = traced_verify(guest, &pi, &child.proof)
                .map_err(|error| PrivacyError::InvalidProof(format!("{error:?}")))?;
            verified.push((pi, summary, ops));
        }
        let mut hints = Hints::default();
        hints.push("fs_seed", lean_vm::cpu::fs_seed(guest).to_vec());
        hints.push(
            "meta",
            vec![
                count(withdrawals.len()),
                count(children.len()),
                F192::new(withdrawals.len() as u64, 0, 0),
            ],
        );
        for id in &ids {
            hints.push("claim_ids", privacy_cells(id).to_vec());
        }
        let mut subs = Vec::with_capacity(children.len());
        let mut carried = Vec::with_capacity(children.len());
        for (child_index, child) in children.iter().enumerate() {
            hints.push(
                "child_count",
                vec![
                    count(child.withdrawals.len()),
                    F192::new(child.withdrawals.len() as u64, 0, 0),
                ],
            );
            let indices = child
                .withdrawals
                ...
                .map(|withdrawal| {
                    withdrawals
                        .binary_search_by_key(&withdrawal.existing_nullifier_hash, |candidate| {
                            candidate.existing_nullifier_hash
                        })
                        .unwrap()
                })
                .collect::<Vec<_>>();
            for index in &indices {
                hints.push("child_index", vec![count(*index)]);
            }
            hints.push("child_defer", child.defer.cells());
            let (sub_hints, defer) = gen_verify(
                guest,
                verified[child_index].0,
                &verified[child_index].1,
                &verified[child_index].2,
            )
            .map_err(|error| PrivacyError::InvalidProof(format!("{error:?}")))?;
            for (name, entry) in sub_hints {
                hints.push(&name, entry);
        ...
        let (agg_hints, defer) = aggregate_deferred_claims(&subs, &carried);
        for (name, entry) in agg_hints {
            hints.push(&name, entry);
        }
        let expected = privacy_statement_digest(&withdrawals, &defer)?;
        let mut program = guest.clone();
        program.min_log_committed = MU_MIN;
        tamper(&mut hints);
        hints.install(&mut program);
        let (proof, _) = prove(&program, expected, log_inv_rate);
        verify(guest, &expected, &proof).map_err(PrivacyError::Vm)?;
        let list_digest = privacy_list_digest(&withdrawals)?;
        Ok(Self {
            withdrawals,
            list_digest,
            statement_digest: digest_bytes(expected),
    ...
    pub fn verify(&self) -> Result<(), PrivacyError> {
        if privacy_list_digest(&self.withdrawals)? != self.list_digest {
            return Err(PrivacyError::InvalidProof("withdrawal list digest mismatch".into()));
        }
        let defer = DeferredClaim::recompute(self.defer.bytecode_point.clone(), self.defer.matrix_point.clone())
            .map_err(|error| PrivacyError::InvalidProof(format!("{error:?}")))?;
        if defer != self.defer {
            return Err(PrivacyError::InvalidProof("deferred claim mismatch".into()));
        }
        let expected = privacy_statement_digest(&self.withdrawals, &self.defer)?;
        if expected != digest_cells(self.statement_digest) {
            return Err(PrivacyError::InvalidProof("statement digest mismatch".into()));
        }
        verify(unified_guest(), &expected, &self.proof)
            .map(|_| ())
            .map_err(PrivacyError::Vm)
    }

    pub fn verify_against(&self, expected: &[WithdrawalPublic]) -> Result<(), PrivacyError> {
        if self.withdrawals != expected {
            return Err(PrivacyError::InvalidList(
                "withdrawal list differs from expected inputs".into(),
            ));
        }
        self.verify()
    }
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        bincode::serialize(&(
            crate::privacy_pool::PROOF_SERIALIZATION_VERSION,
            &self.withdrawals,
            &self.defer.bytecode_point,
            &self.defer.matrix_point,
            &self.proof,
        ))
        .expect("privacy aggregate serializes")
    }

    pub fn to_bytes_without_withdrawals(&self) -> Vec<u8> {
        bincode::serialize(&(
            crate::privacy_pool::PROOF_SERIALIZATION_VERSION,
            &self.defer.bytecode_point,
            &self.defer.matrix_point,
        ...
        .expect("privacy aggregate core serializes")
    }

    pub fn from_bytes(bytes: &[u8]) -> Option<Self> {
        let (version, withdrawals, bytecode_point, matrix_point, proof): (
            u32,
            Vec<WithdrawalPublic>,
            Vec<F192>,
            Vec<F192>,
            lean_vm::cpu::Proof,
        ) = bincode::deserialize(bytes).ok()?;
        if version != crate::privacy_pool::PROOF_SERIALIZATION_VERSION {
            return None;
        }
        let defer = DeferredClaim::recompute(bytecode_point, matrix_point).ok()?;
        let list_digest = privacy_list_digest(&withdrawals).ok()?;
        let statement_digest = digest_bytes(privacy_statement_digest(&withdrawals, &defer).ok()?);
        Some(Self {
            withdrawals,
            list_digest,
            statement_digest,
    ...
    pub fn from_bytes_without_withdrawals(bytes: &[u8], withdrawals: Vec<WithdrawalPublic>) -> Option<Self> {
        let (version, bytecode_point, matrix_point, proof): (u32, Vec<F192>, Vec<F192>, lean_vm::cpu::Proof) =
            bincode::deserialize(bytes).ok()?;
        if version != crate::privacy_pool::PROOF_SERIALIZATION_VERSION {
            return None;
        }
    ...
    pub fn proof_shape_against(
        &self,
        expected: &[WithdrawalPublic],
    ) -> Result<crate::workload::ProofMetadata, PrivacyError> {
        self.verify_against(expected)?;
        self.metadata()
    }

    pub(crate) fn metadata(&self) -> Result<crate::workload::ProofMetadata, PrivacyError> {
        let shape = proof_shape(unified_guest(), &digest_cells(self.statement_digest), &self.proof)
            .map_err(PrivacyError::Vm)?;
        let full = self.to_bytes().len();
        let proof_bytes = bincode::serialized_size(&self.proof).unwrap_or(0) as usize;
        Ok(crate::workload::ProofMetadata {
            program: "crates/rec_aggregation/guests/privacy_pool_withdrawal.py".into(),
            ...
            full_proof_bytes: full,
            proof_without_public_data_bytes: proof_bytes,
            public_data_bytes: full.saturating_sub(proof_bytes),
            public_data_items: self.withdrawals.len(),
...
mod tests {
    use super::*;
    use crate::privacy_pool::fixture;

    #[test]
    fn privacy_guest_reaches_a_fixed_point() {
        let guest = privacy_guest();
        assert!(guest.prog.len().is_power_of_two());
        assert!(!guest.prog.is_empty());
    }

    #[test]
    fn privacy_list_rejects_reordering_and_duplicate_nullifiers() {
        let a = fixture(7, 0).withdrawal.public();
        let b = fixture(7, 1).withdrawal.public();
        let ordered = if a.existing_nullifier_hash < b.existing_nullifier_hash {
            vec![a, b]
        } else {
            vec![b, a]
        };
        let digest = privacy_list_digest(&ordered).unwrap();
        assert_eq!(digest, crate::privacy_pool::withdrawal_list_digest(&ordered).unwrap());
        assert_ne!(digest, privacy_list_digest(&ordered[..1]).unwrap());
        assert!(privacy_list_digest(&[ordered[1], ordered[0]]).is_err());
        let mut duplicate = ordered;
        duplicate[1].existing_nullifier_hash = duplicate[0].existing_nullifier_hash;
        assert!(privacy_list_digest(&duplicate).is_err());
    }

    #[test]
    fn privacy_parent_arity_is_two_through_sixteen() {
        assert!(check_privacy_parent_arity(0).is_err());
        assert!(check_privacy_parent_arity(1).is_err());
        assert!(check_privacy_parent_arity(2).is_ok());
        assert!(check_privacy_parent_arity(16).is_ok());
        assert!(check_privacy_parent_arity(17).is_err());
    }

    #[test]
    fn privacy_statement_binds_all_deferred_claim_limbs() {
        let withdrawal = fixture(7, 0).withdrawal.public();
        let original = DeferredClaim::leaf();
        let expected = privacy_statement_digest(&[withdrawal], &original).unwrap();
        let delta = F192::new(0, 0, 1);
        let mut variants = Vec::new();
        let mut changed = original.clone();
        changed.bytecode_point[0] += delta;
        variants.push(changed);
        let mut changed = original.clone();
        changed.bytecode_value += delta;
        variants.push(changed);
        let mut changed = original.clone();
        changed.matrix_point[0] += delta;
        variants.push(changed);
        let mut changed = original.clone();
        changed.matrix_a_value += delta;
        variants.push(changed);
        let mut changed = original;
        changed.matrix_b_value += delta;
        variants.push(changed);
        assert!(
            variants
                .iter()
                .all(|defer| privacy_statement_digest(&[withdrawal], defer).unwrap() != expected)
        );
    }

    #[test]
    #[ignore = "builds a full self-recursive privacy leaf proof"]
    fn privacy_pool_leaf_proves_and_verifies() {
        lean_vm::init_prover_pool();
        let item = fixture(7, 0);
        let leaf = PrivacyAggregate::prove_leaf(&item.withdrawal, 1).unwrap();
        leaf.verify_against(&[item.withdrawal.public()]).unwrap();
    }

    #[test]
    #[ignore = "builds two leaves and a recursive privacy parent proof"]
    fn privacy_pool_two_leaf_parent_verifies() {
        lean_vm::init_prover_pool();
        let a = fixture(7, 0);
        let b = fixture(7, 1);
        let leaves = [
            PrivacyAggregate::prove_leaf(&a.withdrawal, 1).unwrap(),
            PrivacyAggregate::prove_leaf(&b.withdrawal, 1).unwrap(),
        ];
        let parent = PrivacyAggregate::aggregate(&leaves, 1).unwrap();
        let a_public = a.withdrawal.public();
        let b_public = b.withdrawal.public();
        let expected = if a_public.existing_nullifier_hash < b_public.existing_nullifier_hash {
            vec![a_public, b_public]
        } else {
            vec![b_public, a_public]
        };
        parent.verify_against(&expected).unwrap();
        assert!(parent.verify_against(&expected[..1]).is_err());
        let mut reordered = expected.clone();
        reordered.reverse();
        assert!(parent.verify_against(&reordered).is_err());
        let mut duplicate = expected.clone();
        duplicate.push(expected[0]);
        assert!(parent.verify_against(&duplicate).is_err());

        let full_bytes = parent.to_bytes();
        PrivacyAggregate::from_bytes(&full_bytes)
            .unwrap()
            .verify_against(&expected)
            .unwrap();
        let core_bytes = parent.to_bytes_without_withdrawals();
        PrivacyAggregate::from_bytes_without_withdrawals(&core_bytes, expected.clone())
            .unwrap()
            .verify_against(&expected)
            .unwrap();
        let mut wrong_version = full_bytes;
        wrong_version[..4].copy_from_slice(&0u32.to_le_bytes());
        assert!(PrivacyAggregate::from_bytes(&wrong_version).is_none());
    }

    #[test]
    #[ignore = "builds three recursive privacy aggregation levels"]
    fn privacy_pool_three_levels_discharge_deferred_claims() {
        lean_vm::init_prover_pool();
        let fixtures = (0..4).map(|index| fixture(7, index)).collect::<Vec<_>>();
        let leaves = fixtures
            .iter()
            .map(|item| PrivacyAggregate::prove_leaf(&item.withdrawal, 1).unwrap())
            .collect::<Vec<_>>();
        let left = PrivacyAggregate::aggregate(&leaves[..2], 1).unwrap();
        let right = PrivacyAggregate::aggregate(&leaves[2..], 1).unwrap();
        let root = PrivacyAggregate::aggregate(&[left, right], 1).unwrap();
        ...
        root.verify_against(&expected).unwrap();
    }

    #[test]
    #[ignore = "builds valid children and rejects corrupted parent hint streams"]
    fn privacy_recursive_parent_hints_are_constrained() {
        lean_vm::init_prover_pool();
        let leaves = (0..2)
            .map(|index| PrivacyAggregate::prove_leaf(&fixture(7, index).withdrawal, 1).unwrap())
            .collect::<Vec<_>>();
        type Tamper<'a> = (&'a str, &'a dyn Fn(&mut Hints));
        let cases: &[Tamper] = &[
            ("claim_ids", &|h| h.entries("claim_ids")[0][0] += F192::ONE),
            ("child_index", &|h| h.entries("child_index")[0][0] += F192::ONE),
            ("child_count", &|h| h.entries("child_count")[0][0] += F192::ONE),
            ("child count word", &|h| h.entries("child_count")[0][1] += F192::ONE),
            ("child_defer c0", &|h| h.entries("child_defer")[0][0] += F192::ONE),
            ("child_defer c1", &|h| {
                h.entries("child_defer")[0][0] += F192::new(0, 1, 0)
            }),
            ("child_defer c2", &|h| {
                h.entries("child_defer")[0][0] += F192::new(0, 0, 1)
            }),
            ("fs_seed", &|h| h.entries("fs_seed")[0][0] += F192::ONE),
            ("meta", &|h| h.entries("meta")[0][0] += F192::ONE),
        ];
        for (name, tamper) in cases {
            let outcome = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                PrivacyAggregate::aggregate_tampered(&leaves, 1, |hints| tamper(hints))
                    .map(|proof| proof.verify().is_ok())
            }));
            assert!(!matches!(outcome, Ok(Ok(true))), "tampering {name} must be rejected");
        }
    }

    #[test]
    #[ignore = "builds failing privacy leaves to check application hint constraints"]
    fn privacy_recursive_leaf_hints_are_constrained() {
        lean_vm::init_prover_pool();
        let withdrawal = fixture(7, 0).withdrawal;

        fn reject(withdrawal: &Withdrawal, tamper: impl FnOnce(&mut Program)) {
            let outcome = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                prove_privacy_leaf_tampered(withdrawal, 1, tamper).map(|proof| proof.verify().is_ok())
            }));
            assert!(!matches!(outcome, Ok(Ok(true))));
        }

        reject(&withdrawal, |program| {
            program.set_witness(
                "remaining",
                vec![crate::privacy_pool::U128(withdrawal.remaining.0 ^ 1).cells().to_vec()],
            );
        });
        reject(&withdrawal, |program| {
            program.set_witness("state_bits", vec![privacy_bits(withdrawal.state_path.index ^ 1)]);
        });
        reject(&withdrawal, |program| {
            let mut siblings = withdrawal.state_path.siblings.clone();
            siblings[0][0] ^= 1;
            program.set_witness(
                "state_siblings",
                vec![siblings.iter().flat_map(privacy_cells).collect()],
            );
        });
        reject(&withdrawal, |program| {
            program.set_witness("state_inv_0", vec![vec![F192::ZERO; 32]]);
        });
        reject(&withdrawal, |program| {
            program.set_witness("existing_high", vec![vec![F192::new(2, 0, 0)]]);
        });
        reject(&withdrawal, |program| {
            let mut value = withdrawal.withdrawn.cells().to_vec();
            value[1] = F192::ONE;
            program.set_witness("withdrawn", vec![value]);
        });
    }
}
