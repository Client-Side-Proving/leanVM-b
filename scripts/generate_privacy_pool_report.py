...
<main class="shell">
  ...
  <section class="tab-panel" id="overview" role="tabpanel">
    ...
    <section class="section">
      ...
      <div class="legend"><span><i class="dot" style="background:var(--green)"></i>Meets every selected limit</span><span><i class="dot" style="background:var(--red)"></i>Exceeds this chart's limit</span><span><i class="dot" style="background:var(--muted)"></i>Within this chart's limit, exceeds another selected limit</span><span><i class="ring"></i>Selected configuration</span></div>
    ...
    <p class="footnote">Each result covers six block arrivals, 12 seconds apart, at 0, 12, 24, 36, 48, and 60 seconds. At each arrival, all proofs for that block are supplied together; a root does not wait for proofs from later blocks. The unfinished-root count is measured at 72 seconds. A configuration is admitted when every measured root meets the selected input-to-root limit and no roots remain unfinished at 72 seconds. Ethereum inclusion of the root is outside this benchmark. Child proofs are prepared before timing starts. All serialized child proofs are counted as downloads. Each serialized root proof is counted once per recipient as an upload. The benchmark calculates network requirements from proof sizes; it does not transmit data over a network.</p>
  </section>
...
function checks(row){const network=networkMetrics(row);return[
  ...
  {label:"Unfinished roots after six block arrivals",measured:row.backlogAtNextBlock,limit:0,unit:"roots",digits:0,pass:row.backlogAtNextBlock===0},
...
function resourceMetrics(){return[
  ...
  {key:"memory",title:"Peak RSS",unit:"GiB",digits:2,logarithmic:2,value:row=>row.peakRssGiB,limit:()=>state.ramGiB},
  {key:"backlog",title:"Unfinished roots after six block arrivals",unit:"roots",digits:0,value:row=>row.backlogAtNextBlock,limit:()=>0},
...
function resourceTickValue(metric,value){return metric.key==="memory"?fmt(value,0):resourceValue(metric,value);}
function resourceChart(target,metric,selected){
  ...
  let y,ticks;
  if(metric.logarithmic===2){const positive=values.filter(value=>value>0),minimum=positive.length?Math.min(...positive):1,maximum=positive.length?Math.max(...positive):2,minPower=Math.floor(Math.log2(minimum)),maxPower=Math.max(minPower+1,Math.ceil(Math.log2(maximum))),logLow=minPower,logHigh=maxPower;y=value=>margin.top+(logHigh-Math.log2(Math.max(value,2**minPower)))/(logHigh-logLow)*innerH;ticks=Array.from({length:maxPower-minPower+1},(_,index)=>2**(minPower+index));}
  else if(metric.logarithmic){const positive=values.filter(value=>value>0),low=positive.length?Math.max(.001,Math.min(...positive)*.7):.001,high=positive.length?Math.max(...positive)*1.2:1,logLow=Math.log10(low),logHigh=Math.log10(high);y=value=>margin.top+(logHigh-Math.log10(Math.max(value,low)))/(logHigh-logLow)*innerH;ticks=[];for(let power=Math.floor(logLow);power<=Math.ceil(logHigh);power++){const value=10**power;if(value>=low&&value<=high)ticks.push(value);}}
  ...
  ticks.forEach(tick=>{svg+=`<line class="grid" x1="${margin.left}" x2="${width-margin.right}" y1="${y(tick)}" y2="${y(tick)}"/><text class="axis-text" x="${margin.left-8}" y="${y(tick)+4}" text-anchor="end">${resourceTickValue(metric,tick)}</text>`;});
  ...
  rows.forEach(row=>{const value=metric.value(row),jitter=(row.leafRate-2.5)*2+(row.rootRate-1)+(8-row.workers)*.25,cx=x(row.proofs)+jitter,cy=y(value),within=value<=limit,feasible=passes(row),isSelected=selected&&selected.id===row.id,fill=feasible?'var(--green)':within?'var(--muted)':'var(--red)',details=feasible?'meets every selected limit':within?`within this chart's limit; ${failedChecks(row).join("; ")}`:failedChecks(row).join("; ");if(isSelected)svg+=`<circle cx="${cx}" cy="${cy}" r="8.5" fill="none" stroke="var(--blue)" stroke-width="2.5"/>`;svg+=`<circle cx="${cx}" cy="${cy}" r="4.5" fill="${fill}" stroke="var(--surface)" stroke-width="1"><title>${row.proofs} proofs/root · ${row.workers} threads · ${metric.title}: ${resourceValue(metric,value)} ${metric.unit} · ${esc(details)}</title></circle>`;});
  svg+=`<text class="axis-title" x="${margin.left+innerW/2}" y="${height-9}" text-anchor="middle">Proofs aggregated into one block root</text><text class="axis-title" transform="translate(16 ${margin.top+innerH/2}) rotate(-90)" text-anchor="middle">${esc(metric.unit)}${metric.logarithmic===2?' (log₂ scale)':metric.logarithmic?' (log scale)':''}</text></svg>`;
